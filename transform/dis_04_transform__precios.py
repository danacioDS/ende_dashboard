...
def combine_with_empresa_mapping(
    ...
    logger.info("Iniciando combinación con mapeo de empresas (precios)")
    try:
    ...
    for input_file in sorted(base.glob(pat)):
        if not input_file.name.endswith(".xlsx"):
            continue
        match = re.search(r"extracted_precios_c_ret_(\d+)\.xlsx", input_file.name)
        if not match:
            continue
        file_number = match.group(1)
        output_file = output_prefix / f"precios_empresas_{file_number}.xlsx"
        ...
        try:
            ...
            if header_idx is None:
                logger.error(f"No se encontró 'AGENTE' en {input_file}")
                continue
            df_energia = pd.read_excel(input_file, skiprows=list(range(0, header_idx)))
            df_energia.columns = [
                col.strip() if isinstance(col, str) else col for col in df_energia.columns
            ]
            agent_col = next(
                (col for col in df_energia.columns if "agente" in str(col).lower()),
                None,
            )
            if not agent_col:
                logger.error(f"Columna 'AGENTE' no encontrada en {input_file}")
                continue
            if agent_col != "AGENTE":
            ...
            if df_resultado["EMPRESA"].isna().any():
                ...
                logger.warning(f"{len(missing)} agentes sin empresa asignada: {list(missing[:5])}")
        except Exception as e:
            logger.error(f"Error procesando {input_file}: {e}")
            error_file = output_prefix / f"ERROR_{file_number}.xlsx"
            pd.read_excel(input_file).to_excel(error_file, index=False)
            logger.info(f"Copia de error guardada en: {error_file}")


def consolidate_to_long(output_prefix: Path, output_path: Path):
    logger.info("Consolidando a formato largo (precios)")
    archivos = sorted(output_prefix.glob("precios_empresas_*.xlsx"))
    if not archivos:
...
def pivot_and_clean(input_path: Path, output_path: Path, data_output: Path):
    logger.info("Pivotando y limpiando serie de precios")
...
def run_all(base_dir: Path):
    ...
    combine_with_empresa_mapping(
        empresas_file,
        str(base_dir / "downloads_distribucion" / "extracted_precios_c_ret_*.xlsx"),
        pre_data,
    )
    consolidate_to_long(pre_data, preprocess / "serie_temporal_precios_dis.xlsx")
    pivot_and_clean(
        preprocess / "serie_temporal_precios_dis.xlsx",
        preprocess / "serie_precios_cronologica_dis.xlsx",
        data_out / "serie_precios_dis.xlsx",
    )
