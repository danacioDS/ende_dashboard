...
def procesar_archivos(centrales_file: Path, input_pattern: str, output_prefix: Path):
    logger.info("Procesando archivos de peaje (generación)")
    try:
    ...
    for input_file in sorted(base.glob(pat)):
        if not input_file.name.endswith(".xlsx"):
            continue
        match = re.search(r"extracted_peaje_c_iny_(\d+)\.xlsx", input_file.name)
        if not match:
            continue
        file_number = match.group(1)
        output_file = output_prefix / f"peaje_centrales_{file_number}.xlsx"
...
def consolidate_to_long(output_prefix: Path, output_path: Path):
    logger.info("Consolidando a formato largo (peaje generación)")
    archivos = sorted(output_prefix.glob("peaje_centrales_*.xlsx"))
    if not archivos:
...
def pivot_and_aggregate_peaje(input_path: Path, pivot_path: Path, output_path: Path):
    logger.info("Pivotando y agregando peaje (generación)")
...
def run_all(base_dir: Path):
    ...
    procesar_archivos(
        centrales_file,
        str(base_dir / "downloads_generacion" / "extracted_peaje_c_iny_*.xlsx"),
        pre_data,
    )
    consolidate_to_long(pre_data, preprocess / "serie_peaje_filiales_generacion.xlsx")
    pivot_and_aggregate_peaje(
        preprocess / "serie_peaje_filiales_generacion.xlsx",
