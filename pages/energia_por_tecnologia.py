...
CONFIG = get_page_config("energia_por_tecnologia")

st.set_page_config(page_title=CONFIG["title"], layout="wide")
st.title(CONFIG["subtitle"])

file_path = Path(__file__).resolve().parent.parent / CONFIG["file"]
df_full = pd.read_excel(file_path, engine="openpyxl", nrows=1)
candidates = ["TECNOLOGÍA", "TECNOLOGIA", "TIPO", "TEC"]
tech_col = next((c for c in candidates if c in df_full.columns), None)
if not tech_col:
    st.error("Columna de tecnología no encontrada")
    st.stop()
entity_cols = ["CENTRAL", tech_col]
try:
    raw = load_and_normalize(CONFIG["file"], entity_cols, CONFIG["value_pattern"], CONFIG["value_name"])
except Exception:
    raw = load_and_normalize(CONFIG["file"], ["CENTRAL"], CONFIG["value_pattern"], CONFIG["value_name"])

if raw is None:
    st.stop()
df = raw.rename(columns={tech_col: "TECNOLOGIA"}) if tech_col != "TECNOLOGIA" else raw
...
with tab1:
    c1, c2 = st.columns(2)
    with c1:
        st.subheader(f"Evolución de {CONFIG['child_label']}: {child}")
    ...
    if not df_f.empty:
        ...
        fig = plot_participation_bars(df_f, CONFIG["parent_col"], CONFIG["value_name"], total, "Oranges")
        st.plotly_chart(fig, use_container_width=True)
...
show_sidebar_info(df_f, CONFIG["child_label"], CONFIG["child_col"], CONFIG["parent_label"], CONFIG["parent_col"],
                  "Energía Total", total, CONFIG["unit"])
