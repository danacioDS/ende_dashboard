...
CONFIG = get_page_config("energia_por_generador")
...
with tab1:
    ...
    if not df_f.empty:
        ...
        fig = plot_system_bars(sys_data, "FECHA", CONFIG["value_name"], "Evolución del Sistema", color_scale="Viridis")
        st.plotly_chart(fig, use_container_width=True)
        st.metric("Promedio del Sistema", f"{sys_avg:,.2f} {CONFIG['unit']}")
        st.subheader(f"Participación por {CONFIG['parent_label']}")
        fig = plot_participation_bars(df_f, CONFIG["parent_col"], CONFIG["value_name"], total, "Blues")
        st.plotly_chart(fig, use_container_width=True)
