...
CONFIG = get_page_config("precio_potencia_generacion")
...
with tab1:
    ...
    fig = plot_system_bars(sys_data, "FECHA", CONFIG["value_name"], "Evolución del Sistema", color_scale="Oranges")
    st.plotly_chart(fig, use_container_width=True)
    st.metric("Precio Promedio del Sistema", f"{sys_avg:.2f} {CONFIG['unit']}")

with tab2:
    st.header("Análisis Comparativo")
    st.subheader(f"Comparación de {CONFIG['parent_label']}es")
